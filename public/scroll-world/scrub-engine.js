...
function mountScrollWorld(container, config) {
  ...
  function read() {
    ...
    for (let i = 0; i < N; i++) {
      ...
      let cop;
      // Plateau: fade in quickly, HOLD across the section, fade out only near the seam,
      // so text stays present & readable instead of peaking for an instant mid-scene.
      if (i === 0) cop = after ? 0 : smooth(clamp(1 - Math.max(0, pr - 0.74) / 0.2, 0, 1)); // greets on landing, holds
      else if (i === N - 1) cop = before ? 0 : smooth(clamp(pr / 0.18, 0, 1));              // fades in, holds CTA to end
      else cop = (before || after) ? 0 : smooth(clamp(Math.min(pr, 1 - pr) / 0.16, 0, 1));  // hold across the middle
      const c = copies[i];
      c.style.opacity = cop;
  ...
  function raf() {
    const eps = 0.033;   // one seek per ~30fps video frame — raw (non -g8) encodes decode far less
    for (let i = 0; i < NSEG; i++) {
      const s = SEGMENTS[i];

...
class BookingValueDataTable extends DataTable
{
    public function dataTable($query)
    {
        $data = datatables()
            ...
            ->editColumn('created_at', function (BookingValue $bookingvalue) {
                return UtilityFacades::dateTimeFormat($bookingvalue->created_at);
            })
            ...
            ->editColumn('booking_status', function (BookingValue $bookingvalue) {
                if ($bookingvalue->booking_status == "1") {
                    $out = '<span class="p-2 px-3 badge rounded-pill bg-success">' . __('Successfull') . '</span>';
                    return $out;
                } else {
                    $out = '<span class="p-2 px-3 badge rounded-pill bg-danger">' . __('Cancel') . '</span>';
                    return $out;
                }
            })
        ...
        if ($labels != null) {
            ...
            $arr = array_merge(['user', 'type','action','booking_status','created_at'], array_keys($labels));
        } else {
            $arr = array_merge(['user', 'type','action','booking_status','created_at']);
        }
        $data->rawColumns($arr);
        return $data;
    }
    ...
    protected function getColumns($label)
    {
        $columns = [
            ...
            Column::make('booking_status')->title(__('Booking Status')),
        ];
        if ($label != null) {

...
//    }
    ...
    {
        $formTemplates = FormTemplate::where('created_by', \Auth::user()->id)
            ->orWhere('created_by', 1) // Add this line for created_by = 1 condition
            ->whereNotNull('json')
            ->where('status', 1)
            ->get();
...
//    "type": "header",
//    "subtype": "h5",
//    "label": "Get this form pdf on your Whatsapp and Email (optional)",
//    "column": "1"
//  },
//  {
//    "type": "number",
...
//]]';
            ...
            return redirect()->route('forms.design', $form->id)->with('success', __('Form created successfully.'));
        } else {
            return redirect()->back()->with('failed', __('Permission denied.'));
        }
    }
    ...
    public function edit($id)
    {
        ...
        if (\Auth::user()->can('edit-form') && $user->type == $user->roles->first()->name) {
            ...
            return view('form.edit', compact('form', 'roles', 'formUser', 'formUsers', 'formRole', 'formRoles', 'next', 'previous'));
        } else {
        ...
        if ($id) {
            ...
            if ($user->plan_expired_date >= $currentDateTime) {
                if ($form) {
                    ...
                    return view('form.customer_details', compact('form', 'formValue', 'array', 'backgroundImage'));
                }
            }
        }
    }
    public function customer_details_submit(Request $request , $id){

            $email = $request->email;
            // dd($email);
            $country_code = $request->country_code;
            $whatsapp_number = $request->whatsapp_number;
        $formValue = FormValue::where('id' , 3)->first();
...
//            $pdfUrl = 'testing';
            ...
            if(isset($whatsapp_number) && isset($country_code)){

                $to = '+'.$country_code . $whatsapp_number ;

                try {
                    $pdfsend = (new TwilioService())->sendPdfToWhatsApp($to, $pdfUrl);
                } catch (\Exception $e) {
                    Log::error("WhatsApp customer send failed: " . $e->getMessage());
                }
            }
            ...
            // dd($email);
                try {
                    // Send the email
                        ...
                        Mail::to($email)->send(new NewMail($pdfUrl));

                    // Cleare the files
                $this->clearPdfFolder();
                    // Debug statement to check if email sending is attempted
                } catch (\Exception $e) {
                    // Log any errors
                }
        ...
        return view('form.customer_details');
    ...
    private function clearPdfFolder()
    {
        $pdfDirectory = storage_path('app/pdf/form-value/');
    ...
    public function publicFill($id)
    {
        ...
        if ($id) {
            ...
            if ($form) {
                ...
                return redirect()->route('landingPageHome')->with('failed', __('Form not found.'));
    ...
    {   
        ...
        if ($form) {
            ...
            $bccemails              = explode(',', $form->bccemail);
                ...
                $form = Form::where('id' , $formValue->form_id)->first();
                $user = User::where('id' , $form->created_by)->first();
                $to = $user->country_code . $user->phone;
               
                $pdfUrl = null;
                try {
                    $pdfUrl = $formValue->sendPDF($user); // Generate PDF and get its URL
                } catch (\Exception $e) {
                    Log::error("PDF generation failed for form " . $form->id . ": " . $e->getMessage());
                }
                ...
                try {
                    ...
                    Log::error("WhatsApp send failed for form " . $form->id . ": " . $e->getMessage());
                }
...
//            if (UtilityFacades::getsettings('email_setting_enable') == 'on') {
                if ($form->ccemail && $form->bccemail) {
                    try
                    {
                        $recipientsAccepted = Mail::to($form->email)
                            ->cc($form->ccemail)
                            ->bcc($form->bccemail)
                            ->send(new FormSubmitEmail($pdfUrl));
                    ...
                    try {
                        Mail::to($emails)->send(new FormSubmitEmail($pdfUrl) );
                    } catch (\Exception $e) {
                    }
                }
...
//            }
            ...
            if (isset($request->ajax)) {
                return response()->json([
                    ...
                    'pdfUrl' => $pdfUrl // Include the formValue ID in the response
                ], 200);            } else {
                return redirect()->back()->with(['success' => $successMsg , 'formvalue' => $formValue->id]);
            }
        } else {
            if (isset($request->ajax)) {

...
class DocumentGenratorController extends Controller
{
    ...
    public function store(Request $request)
    {
        request()->validate([
            ...
            'change_log_status' => 'nullable|max:191',
            'change_log_json'   => 'nullable|array',
        ]);
        $user       = User::find(\Auth::user()->admin_id);
        $documents  = DocumentGenrator::where('created_by', \Auth::user()->admin_id)->count();
        $plan       = Plan::find($user->plan_id);
        if ($documents < $plan->max_documents) {
            $filename = '';
            ...
            $document->created_by         = Auth::user()->admin_id;
            ...
            return redirect()->route('document.index')->with('failed', __('Your document limit is over, please upgrade plan.'));
        }
    }
    ...
    public function documentDesignMenu(Request $request, $id)
    {
        ...
        $editorData .= '<div id="editorjs" data-json=' . $documentMenu->json . ' data-id=' . $documentMenu->id . '> </div>';
        $id = $documentMenu->id;
        if ($request->html) {
            $documentMenu->html = $request->html;
        } else {
            $documentMenu->html = $editorData;
        }
        if ($documentMenu) {
            if ($request->value) {

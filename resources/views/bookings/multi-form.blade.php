...
@push('script')
        ...
        $(document).ready(function() {
            ...
            totaldropzone.forEach(function(val) {
                var myDropzone = new Dropzone("#" + val, {
                    url: "{{ route('booking.dropzone.upload', $booking->id) }}",
                    headers: {
                    ...
                    init: function() {
                        this.on('success', function(files, response) {
                            ...
                            if (response.success) {
                                showToStr('Done!', response.success, 'success',
                                    '{{ asset('assets/images/notification/ok-48.png') }}',
                                    4000);
                            } else {
                                showToStr('Error!', response.errors, 'danger',
            ...
            videostream.onclick = e => {
                ...
                try {
                    navigator.mediaDevices.getUserMedia(media.gUM).then(_stream => {
                        ...
                        showToStr('Error!', 'Camera device not found. ', 'danger');
                    });
                } catch (err) {
                    showToStr('Error!', 'Camera device not found.', 'danger');
                }
            ...
            fetch('{{ route('videostore') }}', {
                ...
                .then(res =>
                    ...
                    if (d.success) {
                        ...
                        showToStr('Error!', d.message, 'danger');
                    }
                });
        }
    </script>
    <script>
        ...
        function startTimer() {
            if (!stream) {
                showToStr('Error!', __('Camera device not connected. Please check your camera settings.'), 'danger');
                return;
            }

                        ...
                        @if (isset($json->value) || isset($json->values))
                            @if (isset($json->value))
                                ...
                                @elseif ($json->type == 'video')
                                    @if ($json->value)
                                        <td>
                                            {!! Html::link(Storage::url($json->value), __('video'), []) !!}
                                        </td>
                                    @else
                                        <td>null</td>
                                    @endif
                                @elseif ($json->type == 'selfie')
                                    @if ($json->value)
                                        <td>
                                            {!! Html::link(Storage::url($json->value), __('photo'), []) !!}
                                        </td>
                                    @else
                                        <td>null</td>
                                    @endif
                                @elseif ($json->type == 'SignaturePad')
                                    @if ($json->value)
                                        <td>
                                            {!! Html::link(Storage::url($json->value), __('image'), []) !!}
                                        </td>
                                    @else
                                        <td>null</td>
                                    @endif
                                    ...
                                    @if ($imageChoiceJsons)
                                        @foreach ($imageChoiceJsons as $imageChoiceJson)
                                            @if (isset($imageChoiceJson->selected) && $imageChoiceJson->selected == 1)
                                                <td>
                                                    {!! Html::link(Storage::url($imageChoiceJson->image), $imageChoiceJson->label, []) !!}
                                                </td>
                                            @endif
                                        @endforeach
                                    @else
                                ...
                                @if (isset($json->value))
                                    ...
                                    @if (is_array($fileData))
                                        @foreach ($fileData as $key => $subData)
                                            <td>
                                                {!! Html::link(Storage::url($subData), __('image'), []) !!}
                                            </td>
                                        @endforeach
                                    @else
                                        <td>
